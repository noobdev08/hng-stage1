...
const COUNTRIES = {
  ...
  CV: 'Cape Verde',    MW: 'Malawi',         FR: 'France',
  DE: 'Germany',       GB: 'United Kingdom', US: 'United States',
  IN: 'India',         BR: 'Brazil',         AU: 'Australia',
};

const COUNTRY_NAME_TO_ID = Object.entries(COUNTRIES)
...
function buildWhere(params) {
  ...
  if (min_age !== undefined && min_age !== '')
    and.push({ age: { gte: parseInt(min_age) } });
  if (max_age !== undefined && max_age !== '')
    and.push({ age: { lte: parseInt(max_age) } });
...
async function runProfileQuery(params) {
  const {
    page    = '1',
    limit   = '10',
    sort_by = 'created_at',
  ...
  for (const val of Object.values(numericFields)) {
...
export async function getAllProfiles(req, res) {
  try {
    const outcome = await runProfileQuery(req.query);
...
export async function searchProfiles(req, res) {
  try {
    ...
    for (const [name, id] of COUNTRY_NAME_TO_ID) {
    ...
    const outcome = await runProfileQuery({
      ...filters,
      ...(page  && { page }),
      ...(limit && { limit }),
    });
